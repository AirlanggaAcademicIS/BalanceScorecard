		...
		<h2 class="text-center">Monitoring</h2>
			...
			<div class="form-group">
				...
				<option value="">--Pilih--</option>
					<?php
						$q2 = mysqli_query($konek_db,"SELECT DISTINCT date_format(WAKTU_MULAI_PROKER, '%Y') as bulantahun FROM PROKER order by WAKTU_MULAI_PROKER ASC");
    		...
    		<tbody>
      			...
      			<tr>
					...
					if(isset($_POST['tahun'])){
						$tahun = $_POST['tahun'];
						$query = mysqli_query($konek_db,"SELECT * FROM PROKER WHERE STATUS_PROKER='Terlaksana' AND date_format(WAKTU_MULAI_PROKER, '%Y') = '$tahun'");
				...
				<?php
					if(isset($_POST['tahun'])){
						$tahun = $_POST['tahun'];
						$query = mysqli_query($konek_db,"SELECT * FROM PROKER WHERE STATUS_PROKER='Tidak Terlaksana' AND date_format(WAKTU_MULAI_PROKER, '%Y') = '$tahun'");
						while ($data = mysqli_fetch_array($query)){
				...
				<?php
					if(isset($_POST['tahun'])){
						$tahun = $_POST['tahun'];
						$query = mysqli_query($konek_db,"SELECT * FROM PROKER WHERE STATUS_PROKER='Belum Terlaksana' AND date_format(WAKTU_MULAI_PROKER, '%Y') = '$tahun' AND ID_NOTIFIKASI>0");
						while ($data = mysqli_fetch_array($query)){
							echo "<a href=\"kaprodi_viewnotif.php?id1=".$data['ID_NOTIFIKASI']."\">".$data['NAMA_PROKER']."</a>";
							echo "</br>";
							}

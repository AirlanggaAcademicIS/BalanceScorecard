			...
			<table class="table table-striped">
				...
				<tbody>
					<tr>
						<?php
						$i=0;
						$query = mysql_query("SELECT t.perspektif, p.id_proker, p.nama_proker, k.nama, DATE_FORMAT(p.waktu_mulai_proker, '%d/%m/%Y') AS waktu_mulai_proker, DATE_FORMAT(p.waktu_akhir_proker, '%d/%m/%Y') AS waktu_akhir_proker FROM proker p, menunjang m, indikator_tujuan i, karyawan k, tujuan t WHERE p.NIP=k.NIP AND p.id_proker=m.id_proker AND m.id_indikator_tujuan=i.id_indikator_tujuan AND i.id_tujuan=t.id_tujuan");
